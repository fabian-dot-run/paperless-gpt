...
| `LLM_HTTP_TIMEOUT_SECONDS`          | Overall HTTP timeout (in seconds) for outbound LLM requests (OpenAI and Ollama). `0` means no timeout — recommended for slow local models (e.g. Ollama) that can take longer than a minute to respond.                                | No       | 0 (no timeout)             |
...
| `SERVER_READ_TIMEOUT_SECONDS`       | Read timeout (in seconds) for the built-in HTTP server. `0` means no timeout.                                                                                                                  | No       | 0 (no timeout)             |
| `SERVER_READ_HEADER_TIMEOUT_SECONDS`| Read-header timeout (in seconds) for the built-in HTTP server. `0` means no timeout.                                                                                                           | No       | 0 (no timeout)             |
| `SERVER_WRITE_TIMEOUT_SECONDS`      | Write timeout (in seconds) for the built-in HTTP server. `0` means no timeout — keep at `0` (or set generously) so synchronous LLM requests against slow local models are not aborted mid-response. | No       | 0 (no timeout)             |
| `SERVER_IDLE_TIMEOUT_SECONDS`       | Idle (keep-alive) timeout (in seconds) for the built-in HTTP server. `0` means no timeout.                                                                                                     | No       | 0 (no timeout)             |
